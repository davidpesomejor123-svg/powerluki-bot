...
// ============================
// TICKETS E INTERACCIONES (CON RECLAMAR)
// ============================
client.on('interactionCreate', async interaction => {
  ...
  if (interaction.customId.startsWith('ticket_')) {
    ...
    // Botón Reclamar
    if (category === 'reclamar') {
        if (!interaction.member.permissions.has(PermissionsBitField.Flags.ModerateMembers)) {
            return interaction.reply({ content: '❌ Solo el Staff puede reclamar tickets.', ephemeral: true });
        }
        return interaction.reply({ content: `🎟️ El ticket ha sido reclamado por ${interaction.user}.` });
    }
...
client.login(process.env.TOKEN);import 'dotenv/config';
import fs from 'fs';
import express from 'express';
import {
  Client,
