...
const CONFIG = {
  ...
  CANALES: {
    TICKETS_LOG: '📖tickets',            // panel & log tickets
    ...
    NIVELES: '🆙niveles',
    VALORACIONES: '💪valoraciones',     // canal público donde todos valoran (solicitado)
    STAFF_REVIEW: 'valoracion-staff'   // canal privado para que staff valore
  },
...
/* ───────── READY: registra comandos y publica panel de tickets ───────── */
client.once('ready', async () => {
  ...
  // Publicar panel de tickets en canal TICKETS_LOG si no existe un panel similar
  try {
    ...
    const existe = fetched && fetched.find(m => m.author.id === client.user.id && m.embeds.length && m.embeds[0].title && m.embeds[0].title.includes('Centro de Soporte'));
    if (!existe) {
      const embed = new EmbedBuilder()
        ...
        .setImage('') // puedes agregar url de imagen o dejar vacío
  ...
  try {
    if (interaction.isChatInputCommand()) {
      ...
      if (name === 'ticket') {
        ...
        await ticketChannel.send({ content: `${interaction.user} — Gracias por abrir un ticket. Un staff te atenderá.` });
      ...
      if (name === 'sugerencias') {
        ...
        // obtener o crear canal público de valoraciones
      ...
      /* /sugerenciaserver => envía al canal privado del staff para valoración obligatoria */
      if (name === 'sugerenciaserver') {
        ...
        // crear canal staff si no existe (solo visible a staff)
      ...
      /* Otros comandos: nuevo, anuncio, mute, unmute, ban, temban, encuesta ... (mantengo como antes) */
      ...
      // implementación de mute/unmute/ban/temban similar al archivo anterior...
      if (name === 'mute') {
    ...
    if (interaction.isButton()) {
      ...
      if (id.startsWith('open_ticket_')) {
        ...
        const ticketChannel = await guild.channels.create({ name: channelName, type: ChannelType.GuildText, parent: category ? category.id : undefined, permissionOverwrites: [
          { id: guild.roles.everyone, deny: [PermissionsBitField.Flags.ViewChannel] },
          { id: interaction.user.id, allow: [PermissionsBitField.Flags.ViewChannel, PermissionsBitField.Flags.SendMessages, PermissionsBitField.Flags.ReadMessageHistory] },
          { id: CONFIG.STAFF_ROLE_ID, allow: [PermissionsBitField.Flags.ViewChannel, PermissionsBitField.Flags.SendMessages] }
        ]}).catch(()=>null);
    ...
    if (interaction.type === InteractionType.ModalSubmit) {
      ...
      if (cid.startsWith('staff_accept_') || cid.startsWith('staff_deny_')) {
        ...
        const data = suggestionsDB.get(sId);
        if (!data) return interaction.reply({ content: 'Sugerencia no encontrada.', ephemeral: true });
        // publicar resultado en canal valoraciones
